...
class Registry:
    ...
    def leopoldina(cls, base_std, base_client):
        ...
        if not base_cpf.empty:
            ...
            if id == None:
                # Caso o ID do cliente não exista em base_client ou retorne vazio então preencher ID com CPF
        ...
        base_cnpj = base_client.filter(
            regex=(
            r'(?i)'  # Ignorar maiúsculas/minúsculas
            r'^(?:CNPJ[\s_.\\/-]*(?:ASSESSORIA(S)?|CREDOR(ES)?|ESCRIT[ÓO]RIO)?)|'  # "CNPJ" seguido de "ASSESSORIA", "CREDOR" ou "ESCRITÓRIO"
            r'(?:ASSESSORIA(S)?|CREDOR(ES)?|ESCRIT[ÓO]RIO)[\s_.\\/-]*CNPJ$'  # "ASSESSORIA", "CREDOR" ou "ESCRITÓRIO" seguido de "CNPJ"
            ),
            axis=1
        )   
